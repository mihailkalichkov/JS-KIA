...
mindmaps.StaticCanvasRenderer = function () {
    ...
    function drawBranch(node, $parent) {
        ctx.save();
        branchDrawer.render(ctx, node.getDepth(), node.offset.x, node.offset.y, node, $parent, node.branchColor, 1);
        ctx.restore();
    }
    ...
    function prepareNodes(mindmap) {
        ...
        function addProps(node) {
            var lineWidth = mindmaps.CanvasDrawingUtil.getLineWidth(node.getDepth());
            var metrics = mindmaps.TextMetrics.getTextMetrics(node);
    ...
    function renderCanvas(document) {
        ...
        function drawCaptions(node) {
            ...
            if (node.isRoot()) {
                ...
                ctx.fillStyle = "white";
                mindmaps.CanvasDrawingUtil.roundedRect(ctx, 0 - tm.width / 2 - 4, 20 - 4, tm.width + 8, tm.height + 8, 10);
            }
