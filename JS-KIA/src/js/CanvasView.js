...
mindmaps.DefaultCanvasView = function () {
    ...
    this.getLineWidth = function (depth) {
        return mindmaps.CanvasDrawingUtil.getLineWidth(depth); 
    };

    /* Draws the complete map onto the drawing area. This should only be called once for a mind map. */
    ...
    this.scaleMap = function () {
        ...
        $text.css(
            {
                "font-size": 100 + "%", "left": -mindmaps.TextMetrics.ROOT_CAPTION_MIN_WIDTH / 2
            }).css(metrics);
